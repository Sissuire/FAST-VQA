...
void ReadCommonVideos(const std::string &fileName, std::vector<cv::Mat> &frames, const unsigned int &num)
{
	...
	float *ptr;
	uchar *buf;
	cv::Mat temp(h, w, CV_32FC1), frame_c, frame_yuv, frame_y, frame_comp[3];
	...
	for (unsigned int nof = 0; nof < num; ++nof)
	{
		...
		v >> frame_c;
		cv::cvtColor(frame_c, frame_yuv, CV_BGR2YUV);
		cv::split(frame_yuv, frame_comp);

		buf = (uchar *)frame_comp[0].data;
...
void SimpleTemporalPooling(std::vector<std::list<double>> &outVec, const std::list<double> &tq)
{
    ...
    {
        //q = GetMeanVal(tq);
        meanVal = GetMeanVal(tq);
        stdev = 0.;
    }
    
    outVec[0].push_back(meanVal);
    outVec[1].push_back(stdev);
